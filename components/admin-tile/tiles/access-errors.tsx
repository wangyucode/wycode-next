...
export const AccessErrors = AdminTile(function () {
  ...
  function getErrors(page = 0) {
    fetch(`https://wycode.cn/api/v1/analysis/errors?size=${SIZE}&page=${page}`)
      .then((res) => res.json())
      .then((res) => {
  ...
  return (
    <>
      <table className="border-collapse">
        <colgroup>
          <col className="border-r border-slate-700/30 dark:border-slate-300/30">
          </col>
          <col className="border-r border-slate-700/30 dark:border-slate-300/30">
          </col>
          <col className="border-r border-slate-700/30 dark:border-slate-300/30">
        ...
        <tbody>
          {errors.map((err: any, index: number) => (
            <tr
              key={index}
              className="text-xs odd:bg-slate-200 odd:dark:bg-slate-800 even:bg-slate-300 even:dark:bg-slate-700"
            >
              <td className="px-2 py-1 text-center">{err.ip}</td>
              <td className="px-2 py-1 text-center break-words">
                {err.status}
              </td>
              <td
              ...
              <td className="px-2 py-1" style={{ maxWidth: "500px" }}>
                {err.agent}
              </td>
              <td className="px-2 py-1" style={{ minWidth: "88px" }}>
      ...
      <p className="text-xs">
        showing {errors.length} of {pagination.total} rows.
      </p>
