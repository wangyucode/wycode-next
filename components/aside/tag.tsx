  ...
  return (
    <div className="my-4 p-2 border rounded border-slate-400/30">
      <h2 className="pb-1 mb-1 text-lg font-semibold border-b border-slate-400/30">
        <TagIcon className="inline h-6 mb-0.5" /> 标签
      </h2>
      <ul className="flex flex-wrap gap-x-2">
        {tags.map(({ params }) => <Tag {...params} key={params.cid}></Tag>)}
      </ul>
    </div>
  );
}
...
function Tag(
  { name, count, cid }: { name: string; count: number; cid: string },
) {
  const color =
    TAG_COLORS[
      Math.floor(Math.random() * TAG_COLORS.length) % TAG_COLORS.length
    ];
  ...
  return (
    <li>
      <Link
        ...
        className={`px-1 py-0.5 rounded ${color} hover:bg-black/20 hover:dark:bg-white/20 text-sm`}
      >
        {name}({count})
      </Link>
    </li>
