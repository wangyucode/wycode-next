...
function Tag({name, count, cid}: {name: string, count: number, cid: string}) {
    ...
    return (
        <li >
            <Link href={`/tag/${cid}`}><a style={{fontSize: `${size}px`, lineHeight: `${size}px`}} className={`px-1 py-0.5 rounded ${color} hover:bg-black/20 hover:dark:bg-white/20 text-sm`}>{name}({count})</a></Link>
        </li>
    );
}